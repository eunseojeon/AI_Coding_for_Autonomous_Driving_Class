...
- 이미지의 윤곽선, 경계, 작은 디테일을 강조하여 **더 뚜렷하게 보이도록 하는** 처리 방법입니다.
- **고주파 통과 필터**로, 이미지 내에서 급격한 밝기 변화(엣지 등)를 강조하고, 흐릿함을 줄여줍니다
- 샤프닝 필터는 일반적으로 **합성곱(Convolution) 연산**을 통해 이미지에 적용됩니다.
- 대표적인 3x3 샤프닝 커널 예시:
```
0  -1   0
-1  5  -1
...
- **다운샘플링**
- - **Max Pooling**: 영역 내 최댓값 추출로 대표 특징만 남김.
- - **Average Pooling**: 영역 내 평균값을 사용해 부드러운 특징 맵 생성.
- **공간 크기 축소**
- - **연산량 감소**: 데이터 크기를 줄여 계산 효율 향상.
- - **과적합 방지**: 불필요한 세부 정보를 제거해 모델의 일반화 능력 향상.
- **위치 불변성**
- - **특징 위치 변화에 강인**: 객체의 위치 변화에도 특징을 잘 인식할 수 있도록 도움.
...
- **고차원 특징 통합**
- - **특징 벡터화**: 앞 계층에서 추출된 특징 맵을 1차원 벡터로 변환(Flatten).
- - **종합적 정보 처리**: 모든 뉴런이 서로 연결되어 다양한 특징을 통합 분석.
- **최종 분류 및 회귀**
- - **클래스 결정**: 소프트맥스(Softmax) 등 활성화 함수를 통해 각 클래스별 확률 계산.
- - **값 예측**: 회귀 문제의 경우 연속적인 값 출력.

---
5️⃣ **입력(Input) 계층**
- **데이터 수용**: 원본 이미지, 텍스트, 음성 등 다양한 형태의 데이터를 네트워크에 전달.
- **전처리**: 정규화(Normalization), 크기 조정(Resizing), 채널 분리(RGB 등) 등 입력 데이터의 품질을 높이기 위한 사전 작업 수행.
---

6️⃣ **출력(Output) 계층**
- 최종 결과 반환: 분류(클래스), 회귀(값), 위치(좌표) 등 문제에 맞는 결과 도출.
---
...
|----|------|
| 입력 | 데이터 수용, 전처리|
| 합성곱 | 로컬 패턴 추출, 가중치 공유, 스트라이드/패딩, 특징 맵 생성 |
| 활성화 함수 | 비선형성 부여, 특징 강조/억제 |
| 풀링 | 다운샘플링, 공간 크기 축소, 위치 불변성 확보 |
| 정규화(선택) | 학습 안정화, 속도 향상 |
| 드롭아웃(선택) | 과적합 방지 |
| 완전 연결 | 특징 통합, 최종 분류/회귀 |
| 출력 | 결과 반환 |
