...
- CNN은 이미지를 입력받아 블러가 있는지 없는지 자동으로 분류하거나, 블러의 정도를 정량적으로 평가하는 데 활용됩니다.
- 기존에는 Laplacian, Sobel 등 엣지 기반 연산자로 블러를 검출했으나, CNN은 복잡한 패턴을 학습해 더 다양한 블러 유형(초점 흐림, 모션 블러 등)을 구분할 수 있습니다.
1. 블러 보정 및 제거(Deblurring)
- CNN은 블러가 적용된 이미지를 입력받아 원본에 가까운 선명한 이미지를 복원하는 데 사용됩니다.
- 전통적인 필터 기반 방법과 달리, CNN은 대규모 데이터로부터 흐림의 패턴과 복원 방법을 학습합니다.
- 최근에는 CNN과 트랜스포머(Transformer)를 결합해 더욱 정교한 복원 성능을 보이기도 합니다.
2.  블러에 강인한 네트워크 학습
- 실제 환경에서는 블러가 포함된 이미지가 많으므로, 블러 이미지로 학습한 CNN은 일반 이미지뿐 아니라 흐릿한 이미지에서도 인식 성능이 높아집니다.
- 연구에 따르면, 블러 이미지를 학습에 포함시키면 CNN이 형태(Shape) 정보에 더 민감해지고, 다양한 노이즈와 왜곡에도 더 강인해집니다.
- 이는 인간 시각 시스템이 흐릿한 환경에서도 물체를 인식하는 방식과 유사한 점이 있습니다.
