...
- **TensorRT** 코드는 ONNX 모델을 변환해 추론을 고속 수행하는 데 초점이 있으며, 딥러닝 모델을 설계·학습·평가·추론까지 전 과정을 담당하는 프레임워크이다.
- **PyTorch** 코드는 모델을 직접 정의하고 학습시키는 데 초점이 있습니다. 학습 완료된 모델을 빠르고 효율적으로 **실서비스에 투입(추론 속도 극대화)**하기 위한 성능 가속화 도구입니다.
